...
exports.TextToSpeechVN = void 0;
const logger_1 = require("../../../shared/utils/logger");
class TextToSpeechVN {
    synthesis = null;
    currentVoice = null;
    speed = 1.0;
    constructor() {
    ...
    initializeTTS() {
        if (typeof globalThis !== 'undefined' && globalThis.window && 'speechSynthesis' in globalThis.window) {
            this.synthesis = globalThis.window.speechSynthesis;
            this.loadVietnameseVoices();
        }
        else {
            logger_1.logger.info('Speech Synthesis API not available, using alternative TTS');
        }
    }
    loadVietnameseVoices() {
        ...
        const vietnameseVoices = voices.filter((voice) => voice.lang.startsWith('vi') || voice.lang.includes('VN'));
        if (vietnameseVoices.length > 0) {
            this.currentVoice = vietnameseVoices[0];
            logger_1.logger.info(`🗣️ Vietnamese voice loaded: ${this.currentVoice.name}`);
        }
        else {
            logger_1.logger.warn('No Vietnamese voices available, using default voice');
        }
    }
    /**
    ...
    async speak(text, options) {
        return new Promise((resolve, reject) => {
            if (!this.synthesis) {
                logger_1.logger.warn('Speech synthesis not available');
                resolve();
                return;
            }
            const utterance = new globalThis.window.SpeechSynthesisUtterance(text);
            ...
            utterance.onend = () => {
                logger_1.logger.info(`🗣️ Finished speaking: "${text.substring(0, 50)}..."`);
                resolve();
            };
            ...
            logger_1.logger.info(`🗣️ Speaking: "${text.substring(0, 50)}..."`);
        });
    }
    /**
     * Stop current speech
    ...
    stop() {
        if (this.synthesis) {
            ...
            logger_1.logger.info('🔇 Speech stopped');
        }
    }
    /**
    ...
    pause() {
        if (this.synthesis) {
            ...
            logger_1.logger.info('⏸️ Speech paused');
        }
    }
    /**
    ...
    resume() {
        if (this.synthesis) {
            ...
            logger_1.logger.info('▶️ Speech resumed');
        }
    }
    /**
     * Set speech speed
    ...
    setSpeed(speed) {
        ...
        logger_1.logger.info(`🔊 Speech speed set to: ${this.speed}`);
    }
    /**
    ...
    getVietnameseVoices() {
        if (!this.synthesis)
            return [];
        const voices = this.synthesis.getVoices();
        return voices.filter((voice) => voice.lang.startsWith('vi') || voice.lang.includes('VN'));
    }
    /**
     * Set voice by name
     */
    setVoice(voiceName) {
        ...
        if (voice) {
            ...
            logger_1.logger.info(`🗣️ Voice changed to: ${voiceName}`);
            return true;
        }
        logger_1.logger.warn(`Voice not found: ${voiceName}`);
        return false;
    }
    /**
     * Generate speech with Vietnamese business pronunciation
     */
    async speakBusinessTerms(text) {
    ...
    numberToVietnamese(num) {
        ...
        if (num < 10)
            return ones[num];
        if (num < 20)
            return 'mười' + (num > 10 ? ' ' + ones[num - 10] : '');
        if (num < 100) {
...
exports.TextToSpeechVN = TextToSpeechVN;
//# sourceMappingURL=text-to-speech-vn.js.map