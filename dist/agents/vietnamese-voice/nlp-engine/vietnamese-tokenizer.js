...
exports.VietnameseTokenizer = void 0;
...
class VietnameseTokenizer {
    ...
    tokenize(text) {
        ...
        logger_1.logger.info(`🔤 Tokenized "${text}" into ${tokens.length} tokens`);
        return tokens;
    }
    /**
    ...
    extractEntities(tokens) {
        ...
        logger_1.logger.info(`🏷️ Extracted ${entities.length} entities`);
        return entities;
    }
    /**
     * Recognize intent from Vietnamese command
     */
    recognizeIntent(tokens) {
        const actionWords = tokens.filter(token => this.businessTerms.get(token.toLowerCase())?.match(/CREATE|SHOW|SEND|APPROVE|REJECT|CHECK|SEARCH|EXPORT|PRINT|SAVE|DELETE|UPDATE/));
        const entityWords = tokens.filter(token => this.businessTerms.get(token.toLowerCase())?.match(/INVOICE|REPORT|CUSTOMER|EMPLOYEE|ORDER|PRODUCT/));
     ...
     */
    addBusinessTerm(vietnamese, category) {
        this.businessTerms.set(vietnamese.toLowerCase(), category.toUpperCase());
        logger_1.logger.info(`📝 Added business term: ${vietnamese} -> ${category}`);
    }
    /**
    ...
    analyzeComplexity(text) {
        ...
        const businessTermCount = tokens.filter(token => this.businessTerms.has(token.toLowerCase())).length;
...
exports.VietnameseTokenizer = VietnameseTokenizer;
//# sourceMappingURL=vietnamese-tokenizer.js.map