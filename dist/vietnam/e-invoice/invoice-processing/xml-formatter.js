...
class XMLFormatter {
     ...
     */
    formatInvoiceToXML(invoice) {
        try {
            const xmlObject = this.createVietnamInvoiceXML(invoice);
            const xmlString = this.builder.build(xmlObject);
            logger_1.logger.info(`✅ Invoice ${invoice.invoiceNumber} formatted to XML`);
            return xmlString;
        }
        catch (error) {
            logger_1.logger.error('Failed to format invoice to XML:', error);
     ...
     */
    parseXMLToInvoice(xmlString) {
        try {
            const parsed = this.parser.parse(xmlString);
            const invoice = this.extractInvoiceFromXML(parsed);
            logger_1.logger.info(`✅ XML parsed to invoice object`);
            return invoice;
        }
        catch (error) {
            logger_1.logger.error('Failed to parse XML to invoice:', error);
    ...
    createVietnamInvoiceXML(invoice) {
        return {
            Invoice: {
                '@_version': '1.0',
    ...
    extractSeriesFromInvoiceNumber(invoiceNumber) {
     ...
     */
    createTaxDeclarationXML(period, taxData) {
        const xmlObject = {
            TaxDeclaration: {
                '@_version': '1.0',
